...
export default function Admin() {
  ...
  return (
    <div className="app-shell">
      ...
      <p className="page-sub">Manage subjects, semester window, holidays, and cancellations.</p>
      ...
      {loading ? (
        ...
        <div className="admin__grid">
          <SectionInfoCard section={section} sectionId={activeSectionId} onUpdated={(s) => { setSection(s); flash("Saved."); }} setError={setError} />
          <SubjectsCard sectionId={activeSectionId} subjects={subjects} onChange={setSubjects} setError={setError} flash={flash} />
          <HolidaysCard sectionId={activeSectionId} holidays={holidays} onChange={setHolidays} setError={setError} flash={flash} />
          <CancelLectureCard sectionId={activeSectionId} timetable={timetable} setError={setError} flash={flash} />
        </div>
      )}
    </div>
  );
...
function SectionInfoCard({ section, sectionId, onUpdated, setError }) {
  const [date, setDate] = useState(section?.semesterStartDate ? section.semesterStartDate.slice(0, 10) : "");
    ...
    <div className="surface admin__card">
      ...
      <form onSubmit={handleSave}>
        <div className="field">
          ...
          <input id="semStart" type="date" value={date} onChange={(e) => setDate(e.target.value)} required />
        </div>
        <p className="text-ghost" style={{ fontSize: "0.8rem", marginBottom: 12 }}>
          Required before attendance sync and stats will work.
        </p>
...
function SubjectsCard({ sectionId, subjects, onChange, setError, flash }) {
  ...
  async function handleAdd(e) {
    ...
    setError(null);
    try {
      const res = await createSubject(sectionId, { name: name.trim(), semesterTotal: semTotal ? Number(semTotal) : undefined });
  ...
  async function handleDelete(subjectId) {
    if (!window.confirm("Delete this subject? This will also remove all its attendance records.")) return;
    setError(null);
    try {
  ...
  async function handleSemTotalBlur(subjectId, value) {
    setError(null);
    try {
      const res = await updateSubject(sectionId, subjectId, { semesterTotal: value === "" ? null : Number(value) });
  ...
  return (
    <div className="surface admin__card">
        ...
        {subjects.map((s) => (
          <div key={s.id} className="admin__list-row">
            ...
            <input type="number" className="admin__inline-input" placeholder="Sem total"
              defaultValue={s.semesterTotal ?? ""} onBlur={(e) => handleSemTotalBlur(s.id, e.target.value)}
      ...
      <form onSubmit={handleAdd} className="admin__inline-form">
        <input placeholder="Subject name" value={name} onChange={(e) => setName(e.target.value)} required style={{ flex: 1 }} />
        <input type="number" placeholder="Sem total" value={semTotal} onChange={(e) => setSemTotal(e.target.value)} style={{ width: 90 }} />
...
function HolidaysCard({ sectionId, holidays, onChange, setError, flash }) {
  ...
  async function handleAdd(e) {
    ...
    try {
      ...
      onChange([...holidays.filter((h) => h.date.slice(0, 10) !== date), res.holiday]
  ...
  return (
    <div className="surface admin__card">
      ...
      <div className="admin__list admin__list--scroll">
        ...
        {holidays.map((h) => (
          <div key={h.id} className="admin__list-row">
            <span className="mono">{h.date.slice(0, 10)}</span>
      ...
      <form onSubmit={handleAdd} className="admin__inline-form admin__inline-form--wrap">
        ...
        <input placeholder="Holiday name" value={name} onChange={(e) => setName(e.target.value)} required style={{ flex: 1, minWidth: 120 }} />
...
function CancelLectureCard({ sectionId, timetable, setError, flash }) {
  ...
  const daySlots = timetable.filter((s) => s.dayOfWeek === dayOfWeek && !s.isBreak);
  const selectedSlot = daySlots.find((s) => s.id === timetableSlotId);
  const subjectOptions = selectedSlot
    ? [
        selectedSlot.subject ? { id: selectedSlot.subject.id, label: `${selectedSlot.subject.name} (lecture)` } : null,
        ...selectedSlot.labSlots.filter((l) => l.subject).map((l) => ({
          id: l.subject.id,
          label: `${l.subject.name} (lab B${l.batchNumber})`,
        })),
      ].filter(Boolean)
    : [];
  ...
  async function handleSubmit(e) {
    ...
    setError(null);
    try {
      await createCancellation(sectionId, { date, timetableSlotId, subjectId, reason: reason || undefined });
      flash("Lecture cancelled. Find a make-up slot on the Reschedules tab.");
      setTimetableSlotId(""); setSubjectId(""); setReason("");
    } catch (err) { setError(err.message); }
    finally { setSubmitting(false); }
  }
  ...
  return (
    <div className="surface admin__card admin__card--wide">
      ...
      <p className="text-ghost" style={{ fontSize: "0.85rem", marginBottom: 16 }}>
        Use this when a teacher cancels class. You can find a reschedule slot afterward on the Reschedules page.
      </p>
      <form onSubmit={handleSubmit} className="admin__cancel-form">
        <div className="field">
          <label>Date</label>
          <input type="date" value={date} onChange={(e) => { setDate(e.target.value); setTimetableSlotId(""); setSubjectId(""); }} required />
        </div>
        <div className="field">
          <label>Time slot</label>
          <select value={timetableSlotId} onChange={(e) => { setTimetableSlotId(e.target.value); setSubjectId(""); }} required disabled={dayOfWeek === null}>
            <option value="">{dayOfWeek === null ? "Weekend — no classes" : "Select a slot"}</option>
            {daySlots.map((s) => (
              <option key={s.id} value={s.id}>
                Slot {s.slotIndex + 1} · {TIME_SLOTS[s.slotIndex]}{s.subject ? ` · ${s.subject.name}` : ""}
              </option>
            ))}
          </select>
        </div>
        <div className="field">
          <label>Subject</label>
          <select value={subjectId} onChange={(e) => setSubjectId(e.target.value)} required disabled={!selectedSlot}>
        ...
        <div className="field">
          ...
          <input value={reason} onChange={(e) => setReason(e.target.value)} placeholder="e.g. Faculty on leave" />
        </div>
        <button className="btn btn--primary" type="submit" disabled={submitting || !subjectId}>
